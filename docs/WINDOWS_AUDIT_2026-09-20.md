...
| Instalacja | Dodano `deploy/install.ps1`, `deploy/update.ps1` i `deploy/uninstall.ps1`; instalator sprawdza Git Bash, tworzy katalogi w `ProgramData`, instaluje dwa binaria i rejestruje usługi. Odinstalowanie bez `-Purge` zachowuje dane. | Przegląd statyczny skryptów; pełne uruchomienie wymaga Windows z uprawnieniami administratora. |
...
Nie wykonano natywnego uruchomienia `.exe`, testu Windows Service, testu Job Objects ani świeżej instalacji na czystym Windows w sandboxie Linux. Brak lokalnego Go w obrazie bazowym został obejściowo pokryty tymczasowym Go 1.27.1. Implementacja Job Objects została dodana po tym audycie i wymaga natywnej walidacji na `windows-latest`.

## Ograniczenia i ryzyka pozostające po tej iteracji

1. **Job Objects nie zostały jeszcze natywnie zweryfikowane.** Runner przypina proces do Job Object i używa `TerminateJobObject`; trzeba potwierdzić na Windows, że potomkowie Git Bash są obejmowani oraz że działanie jest zgodne z polityką procesu usługi.
...
4. **Dokumentacja publiczna nie została jeszcze w całości przepisana.** W repozytorium pozostały historyczne sekcje i kod nieużywany w kompilacji Windows odnoszący się do Linux/macOS, `systemd`, `launchd`, `/var` oraz `/etc`.
...
Ta gałąź pozostaje **iteracją naprawczą, nie pełnym zakończeniem migracji**. Po uzupełnieniu spełnia kompilację `windows/amd64`, wymusza Git Bash, używa Job Objects, dodaje Windows Service/PowerShell jako ścieżkę produktu i ustanawia bramkę CI. Nie spełnia jeszcze kryterium natywnej instalacji Windows, pełnych testów SCM/Job Objects oraz testów recovery po twardym zakończeniu executora.
