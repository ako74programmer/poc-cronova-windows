...
applyTheme();
document.querySelectorAll(".nav-item[data-nav]").forEach((n) => n.onclick = () => { const v = n.dataset.nav; v === "pools" ? showPools() : v === "graph" ? showGraph() : v === "resources" ? showResources() : v === "ai-providers" ? showAIProviders() : v === "aiwiki" ? showAIWiki() : v === "audit" ? showAudit() : v === "workers" ? showWorkers() : v === "api" ? showApi() : v === "help" ? window.open(DOCS_URL, "_blank", "noopener") : loadDags(); });
