...
// ---- AI providers (OpenAI-compatible local proxy) ----
let AIP = null;
async function showAIProviders() {
  view = "ai-providers"; activeDag = null; closeLog(); stopDagRunsPoll(); setNav("ai-providers"); setHash("#/ai-providers");
  try { AIP = await api("/api/ai-providers"); }
  catch (e) { main.innerHTML = `<div class="empty err">${t("api_err")}: ${esc(e.message)}</div>`; finishRouteRender(); return; }
  renderAIProviders();
  finishRouteRender();
}
function renderAIProviders() {
  if (view !== "ai-providers" || !AIP) return;
  const admin = !authUser || authUser.role === "admin";
  const rows = AIP.map((p) => `<tr>
      <td class="mono">${esc(p.id)}${p.default ? ` <span class="tag ok">${t("ai_default")}</span>` : ""}</td>
      <td>${esc(p.name || "—")}</td>
      <td class="mono muted">${esc(p.base_url)}</td>
      <td class="mono muted">${esc(p.model)}</td>
      <td>${p.has_token ? `<span class="mono">••••••</span>` : `<span class="muted">—</span>`}</td>
      <td style="font-size:12.5px" class="muted">${fmt(p.updated_at)}</td>
      ${admin ? `<td style="white-space:nowrap"><button data-aiedit="${esc(p.id)}">${t("set_edit")}</button> <button class="icon danger" data-aidel="${esc(p.id)}" aria-label="${t("btn_delete")}">✕</button></td>` : "<td></td>"}
    </tr>`).join("");
  main.innerHTML = `
    <div class="page-h"><h1>${t("ai_title")}</h1><span class="num">${AIP.length}</span></div>
    <div class="page-sub">${esc(t("ai_sub"))}</div>
    ${AIP.length ? `<table class="tbl"><thead><tr>
      <th style="width:180px">${t("ai_id")}</th>
      <th>${t("ai_name")}</th>
      <th>${t("ai_url")}</th>
      <th>${t("ai_model")}</th>
      <th>${t("ai_token")}</th>
      <th style="width:170px">${t("ag_updated")}</th>
      <th style="width:130px"></th>
    </tr></thead><tbody>${rows}</tbody></table>` : `<div class="empty">${t("ai_none")}</div>`}
    ${admin ? `<div class="toolbar" style="margin-top:16px"><button class="primary" id="ai-add">${t("ai_add")}</button></div>` : ""}`;
  if (admin) {
    main.querySelectorAll("[data-aiedit]").forEach((b) => b.onclick = () => aiProviderDialog(AIP.find((p) => p.id === b.dataset.aiedit)));
    main.querySelectorAll("[data-aidel]").forEach((b) => b.onclick = () => delAIProvider(b.dataset.aidel));
    const add = $("ai-add"); if (add) add.onclick = () => aiProviderDialog(null);
  }
}
async function delAIProvider(id) {
  if (!(await confirmDialog(t("ai_del_title", id), t("del_body"), { danger: true, okLabel: t("btn_delete") }))) return;
  try { await api(`/api/ai-providers/${encodeURIComponent(id)}`, { method: "DELETE" }); AIP = AIP.filter((p) => p.id !== id); renderAIProviders(); toast(t("ai_deleted"), "ok"); }
  catch (e) { toast(e.message, "fail"); }
}
function aiProviderDialog(p) {
  const isEdit = !!p;
  const c = p || { id: "default", name: "", base_url: "http://127.0.0.1:4141/v1", model: "", token: "", default: true };
  const root = $("modal-root");
  root.innerHTML = `<div class="overlay" id="aiovl"><div class="modal" role="dialog" aria-modal="true" aria-label="${isEdit ? t("ai_edit") : t("ai_add")}">
    <h2>${isEdit ? t("ai_edit") : t("ai_add")}</h2>
    <div class="body">
      <div class="b-field"><label>${t("ai_id")}</label><input id="ai-id" class="mono" value="${esc(c.id)}" ${isEdit ? "disabled" : ""} placeholder="default"></div>
      <div class="b-field"><label>${t("ai_name")}</label><input id="ai-name" value="${esc(c.name || "")}" placeholder="${esc(t("ai_name_ph"))}"></div>
      <div class="b-field full"><label>${t("ai_url")}</label><input id="ai-url" class="mono" value="${esc(c.base_url || "")}" placeholder="${esc(t("ai_url_ph"))}"></div>
      <div class="b-grid">
        <div class="b-field"><label>${t("ai_model")}</label><input id="ai-model" class="mono" value="${esc(c.model || "")}" placeholder="${esc(t("ai_model_ph"))}"></div>
        <div class="b-field"><label>${t("ai_token")}</label><input id="ai-token" type="password" value="${esc(c.token || "")}" placeholder="${isEdit && p.has_token ? t("c_pw_keep") : ""}"></div>
      </div>
      <div class="b-field" style="flex-direction:row;align-items:center;gap:10px;margin-top:4px">
        <div class="toggle ${c.default ? "on" : ""}" id="ai-def-toggle" role="switch" tabindex="0" aria-checked="${c.default ? "true" : "false"}"></div>
        <label style="margin:0">${t("ai_default")}</label>
      </div>
      <div class="nd-err" id="ai-err"></div>
    </div>
    <div class="foot"><button id="ai-cancel">${t("nd_cancel")}</button><button class="primary" id="ai-save">${t("ai_save")}</button></div>
  </div></div>`;
  const close = () => { document.removeEventListener("keydown", onKey); root.innerHTML = ""; };
  const onKey = (e) => { if (e.key === "Escape") close(); };
  document.addEventListener("keydown", onKey);
  $("aiovl").onclick = (e) => { if (e.target.id === "aiovl") close(); };
  $("ai-cancel").onclick = close;
  let def = !!c.default;
  const tg = $("ai-def-toggle");
  const toggleDef = () => { def = !def; tg.classList.toggle("on", def); tg.setAttribute("aria-checked", def ? "true" : "false"); };
  tg.onclick = toggleDef;
  tg.onkeydown = (e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); toggleDef(); } };
  $("ai-save").onclick = async () => {
    const id = isEdit ? c.id : $("ai-id").value.trim();
    const err = $("ai-err");
    if (!id) { err.textContent = t("ai_id"); return; }
    if (!CFG_KEY_RE.test(id)) { err.textContent = t("err_key"); return; }
    const url = $("ai-url").value.trim();
    const model = $("ai-model").value.trim();
    const token = $("ai-token").value;
    if (!url) { err.textContent = t("ai_need_url"); return; }
    if (!model) { err.textContent = t("ai_need_model"); return; }
    if (!token && !(isEdit && p.has_token)) { err.textContent = t("ai_need_token"); return; }
    const body = { name: $("ai-name").value.trim(), base_url: url, model, token, default: def };
    try {
      const resp = await api(`/api/ai-providers/${encodeURIComponent(id)}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const i = AIP.findIndex((x) => x.id === id);
      if (i >= 0) AIP[i] = resp; else { AIP.push(resp); AIP.sort((a, b) => (a.id < b.id ? -1 : 1)); }
      // only one default provider: reflect server-side normalization in the local list
      if (def) AIP.forEach((x) => { if (x.id !== id) x.default = false; });
      close(); renderAIProviders(); toast(t("ai_saved"), "ok");
    } catch (e) { err.textContent = e.message; }
  };
  $(isEdit ? "ai-name" : "ai-id").focus();
}
