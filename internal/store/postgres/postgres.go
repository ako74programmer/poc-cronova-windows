...
const aiProviderCols = `id, name, base_url, model, token, is_default, created_at, updated_at`

func scanAIProvider(sc scanner) (*model.AIProvider, error) {
	var p model.AIProvider
	var def int
	var cre, upd string
	if err := sc.Scan(&p.ID, &p.Name, &p.BaseURL, &p.Model, &p.Token, &def, &cre, &upd); err != nil {
		return nil, err
	}
	p.Default = def != 0
	p.CreatedAt = parseLoose(cre)
	p.UpdatedAt = parseLoose(upd)
	return &p, nil
}

func (s *Store) ListAIProviders(ctx context.Context) ([]*model.AIProvider, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT `+aiProviderCols+` FROM ai_providers ORDER BY id`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := s.openAIProviderSecret(p); err != nil {
			return nil, err
		}
		out = append(out, p)
	}
...
func (s *Store) GetAIProvider(ctx context.Context, id string) (*model.AIProvider, error) {
	p, err := scanAIProvider(s.db.QueryRowContext(ctx, `SELECT `+aiProviderCols+` FROM ai_providers WHERE id=$1`, id))
	if errors.Is(err, sql.ErrNoRows) {
		return nil, store.ErrNotFound
	}
	if err != nil {
...
func (s *Store) UpsertAIProvider(ctx context.Context, p *model.AIProvider) error {
	token := p.Token
	if s.cipher != nil && token != "" {
		sealed, err := s.cipher.Encrypt(token)
		if err != nil {
			return fmt.Errorf("encrypt ai provider token: %w", err)
		}
		token = sealed
	}
	if p.Default {
		if _, err := s.db.ExecContext(ctx, `UPDATE ai_providers SET is_default=0`); err != nil {
			return err
		}
	}
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO ai_providers (id, name, base_url, model, token, is_default, updated_at)
		 VALUES ($1,$2,$3,$4,$5,$6,CURRENT_TIMESTAMP)
		 ON CONFLICT(id) DO UPDATE SET name=excluded.name, base_url=excluded.base_url,
		   model=excluded.model, token=excluded.token, is_default=excluded.is_default, updated_at=CURRENT_TIMESTAMP`,
		p.ID, p.Name, p.BaseURL, p.Model, token, boolToInt(p.Default))
	return err
}

func (s *Store) DeleteAIProvider(ctx context.Context, id string) error {
	res, err := s.db.ExecContext(ctx, `DELETE FROM ai_providers WHERE id=$1`, id)
	if err != nil {
		return err
	}
	if n, _ := res.RowsAffected(); n == 0 {
...
func (s *Store) openAIProviderSecret(p *model.AIProvider) error {
	if s.cipher == nil || p == nil {
		return nil
	}
	plain, err := s.cipher.Decrypt(p.Token)
	if err != nil {
		return fmt.Errorf("ai provider %q: %w", p.ID, err)
	}
	p.Token = plain
	return nil
}
